...
'%ni%' <- Negate('%in%')
#if ("ranger" %ni% installed.packages()) install.packages("ranger")
if(!require("ranger")) {library(ranger, lib.loc="C:/Users/EPI/Documents/R/win-library/3.6")} #load / install+load installr
if(!require("Matrix")) {library(Matrix, lib.loc="C:/Users/EPI/Documents/R/win-library/3.6")} 
if(!require("lattice")) {library(lattice, lib.loc="C:/Users/EPI/Documents/R/win-library/3.6")} 
...
train_x_learner = function(X, W, Y, ipcw, save = TRUE, pfad) {
  ...
  if (save) {
    saveRDS(xf0, paste0(pfad, "/xf0.rds"))
    saveRDS(xf1, paste0(pfad, "/xf1.rds"))
  }
