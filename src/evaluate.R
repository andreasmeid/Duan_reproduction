...
calibration <- function(pred_rr, y, w, t, cens_time, n_bins=5) {
  ...
  quantiles <- find_bin(pred_rr, up=attributes(bins3)$binhi, lo=attributes(bins3)$binlo) -1
  ...
  #4 3 0 1 3 0 4 2 4]
  #[-0.030103324944473913, 0.0019792321195063302, 0.015565000423523887, 0.03053172125144154, 0.06853847245845485]
  
  obs_rr = c()
  for (i in 0:(n_bins-1)) {
    with_rx <- (quantiles == i & w == 1)
    no_rx <- (quantiles == i & w == 0)
    
    kmf_with_rx = with(tempdf[with_rx,], Surv(t, y))
    kmf_no_rx = with(tempdf[no_rx,], Surv(t, y))

    kmfit_with_rx <- survfit(Surv(t[with_rx], y[with_rx])~1)# , data=tempdf[with_rx,]   surv = kmf_with_rx.survival_function_.reset_index()
    #idx = R_searchsorted(kmfit_with_rx$time, v=cens_time) # Find indices where elements should be inserted to maintain order
    idx = R_searchsorted(summary(kmfit_with_rx)$time, v=cens_time) # Find indices where elements should be inserted to maintain order
    obs_with_rx <- 1 - summary(kmfit_with_rx)$surv[min(idx, length(summary(kmfit_with_rx)$surv) - 1)]

    kmfit_no_rx <- survfit(Surv(t[no_rx], y[no_rx])~1)#, data=tempdf[no_rx,]    surv = kmf_with_rx.survival_function_.reset_index()
    #idx = R_searchsorted(kmfit_no_rx$time, v=cens_time) # Find indices where elements should be inserted to maintain order
    idx = R_searchsorted(summary(kmfit_no_rx)$time, v=cens_time) 
    obs_no_rx <- 1 - summary(kmfit_no_rx)$surv[min(idx, length(summary(kmfit_no_rx)$surv) - 1)]

    obs_rr <- c(obs_rr, (obs_no_rx - obs_with_rx))
    #[-0.03466292533119142, -0.0031541886088251037, 0.0065715551849898635, 0.02525238999376589, 0.06977257231020095]
  }
...
# read python results
  if (python==TRUE) {
    require(RcppCNPy)
    pred_rr <- pred_rr_python <-npyLoad(paste0(base_dir, "/pred_rr.npy")) # pred_rr <- pred_rr_python
...
# metrics for the dataset, evaluated as dichotomous outcome
  arr_ben <- arr_noben <- c_stat <- rmst <- slope <- intercept <- c()
  set.seed <- 123
  ...
  if (args.cstat) {
    ...
    c_stat <- unlist(pbreplicate(bootstrap_samples, evaluate_bootstrap_disc("c_for_benefit")))
    #c_stat <- unlist(replicate(bootstrap_samples, evaluate_bootstrap_disc("c_for_benefit"), simplify=FALSE))
  }
  arr_ben <- unlist(replicate(bootstrap_samples, evaluate_bootstrap_disc("arr_ben"), simplify=FALSE))
  arr_noben <- unlist(replicate(bootstrap_samples, evaluate_bootstrap_disc("arr_noben"), simplify=FALSE))
...
# metrics for the dataset, evaluated on the entire sample  
  evaluate_bootstrap_cal <- function(metric="c_for_benefit") {
    ...
    if (metric=="rmst") {
      ret <- decision_value_rmst(pred_rr[idxs], y[idxs],w[idxs], t[idxs],args.cens_time)
    } else {
      cali <- calibration(pred_rr[idxs], y[idxs],w[idxs], t[idxs],args.cens_time, n_bins=5)
      if (metric=="slope") {
      ret <- as.numeric(cali[[1]])
      } else if (metric=="intercept") { 
      ret <- as.numeric(cali[[2]])
      }
    } 
    return(ret)
  }
  ...
  if (args.calc_rmst) {
    pboptions(type = "txt", style = 3, char = "=")
    rmst <- unlist(pbreplicate(bootstrap_samples, evaluate_bootstrap_cal("rmst")))
    
    #rmst <- unlist(replicate(bootstrap_samples, evaluate_bootstrap_cal("rmst"), simplify=FALSE))
    print("rmst")
    print(get_range(rmst)); 
  }
  if (args.calibrat) {
    pboptions(type = "txt", style = 3, char = "=")
    slope <- unlist(pbreplicate(bootstrap_samples, evaluate_bootstrap_cal("slope")))
    #slope <- unlist(replicate(bootstrap_samples, evaluate_bootstrap_cal("slope"), simplify=FALSE))
    pboptions(type = "txt", style = 3, char = "=")
    intercept <- unlist(pbreplicate(bootstrap_samples, evaluate_bootstrap_cal("intercept")))
    print("calibration slope"); print(get_range(slope)); 
    print("calibration intercept");print(get_range(intercept));
  }
...
# metrics for the dataset, non-bootstrapped
  ...
  stats <- list(c_stat=c_stat, rmst=rmst, slope=slope, intercept=intercept,
                arr_ben=arr_ben,arr_noben=arr_noben,
  ...
  for (i in 1:length(stats)) {
    ...
    ob <- stats[[i]]
    if (python) { 
      pfad <- paste0(base_dir, "/", datname, "_py.rds")
    } else {
